...
echo "KrankyBearTailer - Windows Compile (bash)"
echo "================================================"
echo
...
rm -f bin/tailer-windows.exe || true
...
if ! go build -ldflags="$LDFLAGS" -trimpath -o bin/tailer-windows.exe
then
...
then
  ...
  cp -f bin/tailer-windows.exe ./KrankyBearTailer.exe
  ISCC_DEFAULT="/c/Program Files (x86)/Inno Setup 6/ISCC.exe"
  if [[ -x "$ISCC_DEFAULT" || -f "$ISCC_DEFAULT" ]]
  then
    "${ISCC_DEFAULT}" ./Inno/KrankyBearTailer.iss || {
